...
$data = array(
'v' => 1, # обязательный неизменный параметр - версия MP
...
// Проверяем статус HTTP - вернул ли запрос код 200
if (!curl_errno($ch)) {
  switch ($http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE)) {
    case 200:  # OK
    echo 'Status of user and transaction updated: '.$http_code;  # в продакшн здесь добавляем в запись в БД метку что данные в GA отправлены
      break;
    default:
      echo 'Unexpected HTTP code: '.$http_code; # в продакшн здесь добавляем в запись в БД метку что данные в GA не отправлены и возвращенный код HTTP
  } 
} else {
